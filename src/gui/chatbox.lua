local chatbox = {}
chatbox.open = false
chatbox.width = 400
chatbox.messages = {}

function chatbox:unload()
	bind:removeBind("sendChat")
end

function chatbox:message(name, msg)
	local lines = math.floor(font.small:getWidth(name .. ": " .. msg) / self.width + 1)
	self.messages[#self.messages + 1] = {name, msg, 0, lines}
end

function chatbox:load()
	self.sendButton = button:new("Send", self.width - 40, screenHeight - font.small:getHeight() - 4, 40, font.small:getHeight(), function()
		self.open = false

		if self.textbox.text == "" then return end

		client:send("CHAT", {game:getLocalPlayer().name, self.textbox.text})
		self.textbox.text = ""
	end)

	self.sendButton.font = font.small
	self.sendButton.color = 		{40,40,40, 140}
	self.sendButton.hoverColor = 	{30,30,30, 140}
	self.sendButton.clickColor = 	{5,5,5, 140}


	self.textbox = textbox:new("", font.small, 2, screenHeight - font.small:getHeight() - 4, self.width - 40)
	self.textbox.activeColor = {10,10,10,140}

	bind:addBind("sendChat", "return", function(down)
		if not down and self.open then
			self.sendButton:onClick()
		end
	end)

	bind:addBind("exitChat", "escape", function(down)
		if not down and self.open then
			self.open = false
		end
	end)
end

function chatbox:draw()
	if not self.open then 
		local y = screenHeight - font.small:getHeight() - 4 - 2

		for i=#self.messages, 1, - 1 do
			if self.messages[i][3] < 5 then
				y = y - font.small:getHeight() * self.messages[i][4]


				local rectAlpha = math.min( self.textbox.activeColor[4], (5 / self.messages[i][3] - 1) * 255)
				local alpha = math.min( 255, (5 / self.messages[i][3] - 1) * 255)
				
				love.graphics.setColor(self.textbox.activeColor[1], self.textbox.activeColor[2], self.textbox.activeColor[3], rectAlpha)
				love.graphics.rectangle('fill', 2, y, self.width, font.small:getHeight() * self.messages[i][4])
				love.graphics.setColor(255,255,255)
				
				love.graphics.printf({ {50, 50, 205, alpha}, self.messages[i][1] .. ": ", {255, 255, 255, alpha}, self.messages[i][2] }, 2, y, self.width)
			end
		end

		return 
	end

	local lines = 0
	local toShow = {}
	for i=#self.messages, 1, -1 do
		if lines + self.messages[i][4] > 5 then
			break
		else
			lines = lines + self.messages[i][4]
			toShow[#toShow + 1] = self.messages[i]
		end
	end

	local y = screenHeight - font.small:getHeight() - 4 - 2

	for i=1, #toShow do
		y = y - font.small:getHeight() * toShow[i][4]

		love.graphics.setColor(self.textbox.activeColor)
		love.graphics.rectangle('fill', 2, y, self.width, font.small:getHeight() * toShow[i][4])
		love.graphics.setColor(255,255,255)
		
		love.graphics.printf({ {50, 50, 205}, toShow[i][1] .. ": ", {255, 255, 255}, toShow[i][2] }, 2, y, self.width)
	end

	self.textbox:draw()
	self.sendButton:draw()
end

function chatbox:update(dt)
	for i=1,#self.messages do
		self.messages[i][3] = math.min(5, self.messages[i][3] + dt)
	end

	if not self.open then return end

	self.textbox:update(dt)
	self.sendButton:update(dt)

	self.textbox.active = true
end

function chatbox:keypressed(key)
	if not self.open then return end

	self.textbox:keypressed(key)
end

function chatbox:textinput(t)
	if not self.open then return end

	self.textbox:textinput(t)
end

return chatbox